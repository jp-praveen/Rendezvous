function [opt_path]= a_starmod(st_pt,end_pt,node_xy,obs_xy,edge,opt_dis)
q = st_pt;
w = end_pt;
...
%past_cost
table=[];
table(1,1)=past_cost(q,1);
table(2,1)=opt_dis(q,1);
table(3,1)= table(1,1)+ table(2,1);
table(4,1)=0;

table(1,n)=past_cost(w,1);
table(2,n)=opt_dis(w,1);
table(3,n)= table(1,n)+ table(2,n);
table(4,n)=0;
...
for i=1:n-2;
    table(1,i+1)=past_cost(i,1);
    table(2,i+1)=opt_dis(i,1);
    table(3,i+1)= table(1,i)+ table(2,i);
    table(4,i+1)=0;
end
table       
open=[];
